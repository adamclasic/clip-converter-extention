// ==UserScript==
// @name        ClipConverter
// @namespace   https://www.clipconverter.cc/
// @version     1.5.3
// @description This userscript assists ClipConverter in fetching download links. 
// @copyright   2015, ClipConverter
// @icon        https://www.clipconverter.cc/images/128.png
// @icon64      https://www.clipconverter.cc/images/appletouchicon.png
// @homepage    https://www.clipconverter.cc/addon
// @updateURL   https://www.clipconverter.cc/extension/clipconverter.meta.js
// @downloadURL https://www.clipconverter.cc/extension/clipconverter.user.js
...
if("undefined" == typeof (clipconverter)) {
	var clipconverter = {
        ...
        userUrl: 'https://www.clipconverter.cc/?ref=addon&version=153&browser=userscript',
        saveUrl: 'https://www.clipconverter.cc/addonsave?version=153&browser=userscript',
		...
		addButtons: function(document) {
				...
				if(document.getElementById('meta-contents')) {
                    ...
                    div_embed.innerHTML += ' <paper-button subscribed id="clipconverter" style="float:right" class="ytd-subscribe-button-renderer"><a href="' + clipconverterpath + '" target="' + target + '" style="text-decoration: none; color: inherit;" class="style-scope ytd-subscribe-button-renderer"><img  style="vertical-align: bottom;" src="' + icon + '"> <strong>ClipConverter</strong></a></paper-button>'+ div_embed.innerHTML;
                ...
                window.addEventListener('message', function(event) {
                    ...
                    if(event.data && event.data.youtube) {
                        ...
                        request.onload = function (response) {
                            if(/>var ytplayer(.*?)<\/script>/i.test(response.responseText)) {
                                window.postMessage({'url' : url, 'player': />var ytplayer(.*?)<\/script>/i.exec(response.responseText)[1], 'code': response.status}, "https://www.clipconverter.cc/");
                            } else {
                                window.postMessage({'url' : url, 'error': 'Player not found', 'code': response.status}, "https://www.clipconverter.cc/");
                            }
                        };
                        request.onerror = function (response) {
                            window.postMessage({'url' : url, 'error': 'network error', 'code': -1}, "https://www.clipconverter.cc/");
                        };
        ...
        check: function() {
            ...
            if(clipconverter.currentMediaUrl != document.URL  && typeof ytplayer != 'undefined' && ytplayer) {
                // new video detected
                clipconverter.currentMediaUrl = document.URL;
                //clipconverter.saveUrlMap();
                if(document.getElementById('clipconverter')) {
                    document.getElementById('clipconverter').outerHTML="";
                }
			}

            if(!document.getElementById('clipconverter') && typeof ytplayer != 'undefined' && ytplayer) {
    ...
    saveUrlMap: function() {
		...
		if(typeof ytplayer != 'undefined' && ytplayer && ytplayer.config &&  ytplayer.config.args && ytplayer.config.args.url_encoded_fmt_stream_map) {
            ...
            gm_xhr.data = "urlmap=" + encodeURIComponent(urlmap) + "&title=" + encodeURIComponent(title) + "&sts=" +  encodeURIComponent(sts) + "&mediaurl=" +  encodeURIComponent(document.URL) + "&channel=" +  encodeURIComponent(channel);
